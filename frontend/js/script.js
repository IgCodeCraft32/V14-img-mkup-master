...
function start(event, tool) {
    if(tool == "free") document.getElementById('drawContainer').addEventListener('mousemove', drawFree);
    if(tool == "text"){
        return;
    }
    isDrawing = true;
...
function stop(event, tool, txt = "") {
    if(tool == "free"){
        document.getElementById('drawContainer').removeEventListener('mousemove', drawFree);
    }
    if(tool == "text"){
...
function drawFree(event) {
   console.log('####### draw lineThickness='+thick+' lineColor='+lineColor);
   ctx.beginPath();
   ctx.lineWidth = thick;
   ctx.lineCap = 'round';
...
window.ns = {
    ...
    setTool: function(tool, txt = "") {
        ...
        document.getElementById('drawContainer').addEventListener('mousedown', function(event) {
            start(event, tool);
        });
        document.getElementById('drawContainer').addEventListener('mouseup', function(event) {
            stop(event, tool, txt);
        });
    },
