...
public class MainView extends VerticalLayout { 
  ...
  public MainView() {
    ...
    startEditingButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY,ButtonVariant.LUMO_SUCCESS,ButtonVariant.LUMO_SMALL);
    ...
    launchViewerButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY,ButtonVariant.LUMO_SUCCESS,ButtonVariant.LUMO_SMALL);
    ...
    tf.addValueChangeListener(val->{
      if (!val.isFromClient()) return;
      textValue = val.getValue().trim();
      launchTextEditor();
    });
    tf.addFocusListener(evt->{
      if (!evt.isFromClient()) return;
      launchTextEditor();
    });
    imgToolBar.add(tf);
